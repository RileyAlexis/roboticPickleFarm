...
import { Button, Typography, Checkbox, Paper } from "@mui/material";
...
function LandingPage() {
  ...
  const processLogin = () => {
    ...
    axios.post("/game/login", dataObj).then((response) => {
      if (response.data.detail) {
        ...
        axios
          ...
          .then((response) => {
            ...
            dispatch({
              type: "stats/setAllStats",
              payload: response.data.stats,
            });
            dispatch({
              type: "resources/setAllResources",
              payload: response.data.resources,
            });
            dispatch({
              type: "robots/setAllBots",
              payload: response.data.robots,
            });
            dispatch({
              type: "upgrades/setAllUpgrades",
              payload: response.data.upgrades,
            });
            dispatch({
              type: "buildings/setAllBuildings",
              payload: response.data.buildings,
            });
            dispatch({
              type: "prices/setAllPrices",
              payload: response.data.prices,
            });
            dispatch({
              type: "plants/setAllPlants",
              payload: response.data.plants,
            });
            dispatch({ type: "RUN_ENGINE" });
  ...
  const createNewUser = () => {
    if (email.length < 5) {
      setError("User Email must be at least 5 characters");
      return;
    }
    if (password !== confirmPassword || password === "") {
      ...
      axios.post("/game/newUser", dataObj).then((response) => {
        if (response.data.detail) {
          setError(response.data.detail);
        } else {
          setCookie("Email", response.data.email);
          ...
          dispatch({
            type: "stats/toggleActive",
            payload: { title: "playGuide" },
          });
        }
      });
    }
      ...
      {login && (
        <div className="loginBox">
          ...
          <Typography variant="caption">
            If no login is provided game data will not be saved(local storage
            not implemented yet)
          </Typography>
        </div>
      )}
